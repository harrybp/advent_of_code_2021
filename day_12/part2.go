...
func checkDuplicates(path []*node) bool {
    for i := 0; i < len(path); i++ {
        for j:= 0; j < len(path); j++ {
            if (i != j) && (path[i].name == path[j].name) && !unicode.IsUpper(rune(path[i].name[0])){
                return true
            }
        }
    }
...
func main() {
    ...
    for len(pathStack) > 0 {
        ...
        for _, item := range newPath[len(newPath)-1].children {
            ...
            } else if item.big || !checkExists(newPath, item) || !checkDuplicates(newPath) {
